...
export function formatDeclarationsOutput(result, options = {}) {
    const lines = [];
    ...
    for (const entry of lines) {
        ...
        if (typeof entry.line === "number") {
            outLines.push(...formatEntryLines(entry.content, `${formatGutterLine(entry.line, width, chalkInstance)} `, `${formatGutterBullet(width, chalkInstance)} `, formatGutterBullet(width, chalkInstance)));
            continue;
        }
        outLines.push(...formatEntryLines(entry.content, `${formatGutterBullet(width, chalkInstance)} `, `${formatGutterBullet(width, chalkInstance)} `, formatGutterBullet(width, chalkInstance)));
    }
...
function formatEntryLines(content, firstPrefix, continuationPrefix, blankLine) {
...
function formatDeclarationLine(decl, chalkInstance) {
    const useColor = chalkInstance.level > 0;
    const kw = (value) => (useColor ? chalkInstance.cyan(value) : value);
    const nm = (value) => (useColor ? chalkInstance.yellow(value) : value);
    const ty = (value) => (useColor ? chalkInstance.green(value) : value);
...
function formatCallableSignature(rawSignature, formatKeyword, formatName, formatType) {
...
function formatMemberLine(member, containerKind, chalkInstance) {
    const useColor = chalkInstance.level > 0;
    const kw = (value) => (useColor ? chalkInstance.cyan(value) : value);
    const nm = (value) => (useColor ? chalkInstance.yellow(value) : value);
    const ty = (value) => (useColor ? chalkInstance.green(value) : value);
    ...
    const hasVisibility = trimmed.startsWith("public ")
...
function highlightMemberSignature(signature, formatKeyword, formatName, formatType) {
...
function renderDocBlock(doc, indent, chalkInstance) {
    const useColor = chalkInstance.level > 0;
    const docColor = useColor ? chalkInstance.gray : (value) => value;
...
function formatClosingBrace(chalkInstance) {
    return chalkInstance.level > 0 ? chalkInstance.cyan("}") : "}";
}
function formatGutterLine(line, width, chalkInstance) {
    const useColor = chalkInstance.level > 0;
    const gutter = `${String(line).padStart(width)}:`;
    return useColor ? chalkInstance.gray(gutter) : gutter;
}
function formatGutterBullet(width, chalkInstance) {
    const useColor = chalkInstance.level > 0;
    const bullet = `${" ".repeat(Math.max(0, width - 1))}•`;
    return useColor ? chalkInstance.gray(bullet) : bullet;
}
function highlightExportedDeclaration(text, formatKeyword, formatName, formatType) {
    let out = text;
    // Highlight `export ... <kind> <Name>` in one pass so ANSI escapes don't break follow-up matches.
    out = out.replace(/\bexport\s+((?:(?:declare|default|async)\s+)*)\b(function|class|interface|enum|type|const)\s+([A-Za-z_$][A-Za-z0-9_$]*)/g, (_m, modifiers, kind, name) => {
    ...
    out = out.replace(/\b(extends|implements|namespace|module|declare|default|async)\b/g, (m) => formatKeyword(m));
...
function formatDoc(symbol, checker) {
...
function extractLeadingFileDoc(sourceFile) {
...
function buildClassMemberInfo(sourceFile, checker, member) {
    ...
    if (modFlags & ts.ModifierFlags.Static)
        modifiers.push("static");
    if (modFlags & ts.ModifierFlags.Async)
        modifiers.push("async");
    if (modFlags & ts.ModifierFlags.Readonly)
...
function buildInterfaceMemberInfo(sourceFile, checker, member) {
    const pos = fromPosition(sourceFile, member.getStart(sourceFile));
    const doc = (() => {
...
function buildDeclarationText(sourceFile, declaration) {
